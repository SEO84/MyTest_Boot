package com.busanit501.foodboot.repository.search;

import com.busanit501.foodboot.domain.Food;
import com.busanit501.foodboot.domain.QFood;
import com.busanit501.foodboot.domain.QReply;
...
public class FoodSearchImpl extends QuerydslRepositorySupport
    ...
    public Page<Food> search(Pageable pageable) {
        ...
        query.where(food.fname.contains("3"));
        // =================================.,조건1

        // 제목, 작성자 검색 조건 추가,
        BooleanBuilder booleanBuilder = new BooleanBuilder();
        booleanBuilder.or(food.fname.contains("3"));// "3" 제목 임시
        booleanBuilder.or(food.fdesc.contains("7"));// "3" 제목 임시
        // query, 해당 조건을 적용함.
            ...
            for (String type : types) {
                switch (type) {
                    case "n":
                        ...
                        break;
                    case "d":
                        booleanBuilder.or(food.fdesc.contains(keyword));
                        break;
                    case "c":
                        booleanBuilder.or(food.chef.contains(keyword));
                        break;
                } // switch
            }// end for
            // where 조건을 적용해보기.
    ...
    public Page<FoodListReplyCountDTO> searchWithReplyCount(String[] types, String keyword, Pageable pageable) {
        ...
        JPQLQuery<Food> query = from(food);// select * from board
